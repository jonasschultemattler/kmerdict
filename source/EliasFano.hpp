...
template <util::AllocType AT = util::AllocType::MALLOC> class EliasFano : public Rank, public Select {
  ...
  public:
    ...
    void save(Archive &ar) const {
    	std::vector<uint64_t> lower_vec(lower_bits.size());
    	std::vector<uint64_t> upper_vec(upper_bits.size());

    	for (size_t i = 0; i < lower_bits.size(); ++i) lower_vec[i] = lower_bits[i];
    	for (size_t i = 0; i < upper_bits.size(); ++i) upper_vec[i] = upper_bits[i];

    	ar(num_bits, num_ones, l, block_size, block_length, block_size_mask, lower_l_bits_mask, lower_vec, upper_vec);
	}

	template <class Archive>
	void load(Archive &ar) {
    	std::vector<uint64_t> lower_vec, upper_vec;
    	ar(num_bits, num_ones, l, block_size, block_length, block_size_mask, lower_l_bits_mask, lower_vec, upper_vec);

    	lower_bits.resize(lower_vec.size());
    	upper_bits.resize(upper_vec.size());

    	for (size_t i = 0; i < lower_vec.size(); ++i) lower_bits[i] = lower_vec[i];
    	for (size_t i = 0; i < upper_vec.size(); ++i) upper_bits[i] = upper_vec[i];

    	// Reconstruct selects
    	select_upper = SimpleSelectHalf<AT>(&upper_bits, num_ones + (num_bits >> l) + 1);
    	selectz_upper = SimpleSelectZeroHalf<AT>(&upper_bits, num_ones + (num_bits >> l) + 1);
