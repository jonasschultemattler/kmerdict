...
public:
    ...
    basic_iterator(range_iterator_t range_iterator,
                   ...
                   xor_minimiser_and_positions_parameters const & params) :
...
private:
    ...
    void rolling_hash()
    {
        ...
        value = (value >> 2) | new_rank << 2*(minimiser_size-1);
        value_rev = ((value_rev << 2) | new_rank^(0b11)) & kmer_mask;
    }
    ...
    void next_window()
    {
        ...
        kmers_in_window.push_back(std::min<uint64_t>(m_hasher.hash(value) & kmer_mask, m_hasher.hash(value_rev) & kmer_mask));
    }
    ...
    void init(xor_minimiser_and_positions_parameters const & params)
    {
        seed = params.seed;
        m_hasher.seed(seed);
        ...
        value |= new_rank << (2 * (minimiser_size - 1));
        value_rev |= new_rank^0b11;
...
struct xor_minimiser_and_positions_fn
{
    ...
    constexpr auto operator()(range_t && range, xor_minimiser_and_positions_parameters params) const
    {
        ...
        return xor_minimiser_and_positions{range, std::move(params)};
    }
};
...
inline constexpr auto xor_minimiser_and_positions = rshash::detail::xor_minimiser_and_positions_fn{};
...
private:
    ...
    void next_window()
    {
        ...
        // const uint64_t kmerhash = std::min<uint64_t>(m_hasher.hash(value), m_hasher.hash(value_rev)) & kmer_mask;
        const uint64_t kmerhash = std::min<uint64_t>(m_hasher.hash(value) & kmer_mask, m_hasher.hash(value_rev) & kmer_mask);
...
inline constexpr auto xor_minimiser_and_skmer_positions = rshash::detail::xor_minimiser_and_skmer_positions_fn{};
