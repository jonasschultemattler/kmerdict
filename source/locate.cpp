    ...
    std::vector<std::pair<uint64_t, bool>> &positions, size_t &found_positions, size_t &found_kmers)
{
    ...
    if(left_minimiser_pos != k-m-right_minimiser_pos) {
        for(size_t i = 0; i < no_minimiser; i++) {
            found |= report_minimiser_pos2<level, use_shape>(buffer, offsets[i], query, queryrc, found_positions, s, left_minimiser_pos, right_minimiser_pos, positions);
            s += span;
            if constexpr (use_shape)
                s += 2*overlap;
        }
        ...
        for(size_t i = 0; i < no_minimiser; i++) {
            found |= report_minimiser_pos<level, use_shape>(buffer, offsets[i], query, queryrc, found_positions, s, left_minimiser_pos, positions);
            s += span;
            if constexpr (use_shape)
                s += 2*overlap;
        }
    ...
    std::vector<std::pair<uint64_t, bool>> &positions, size_t &found_positions)
{
    ...
    if(level == 1) {
        if(use_shape)
            return streaming_locate1<true>(query, positions, found_positions);
        else
            return streaming_locate1<false>(query, positions, found_positions);
    }
    else if(level == 2) {
        if(use_shape)
            return streaming_locate2<true>(query, positions, found_positions);
        else
            return streaming_locate2<false>(query, positions, found_positions);
    }
    else if(level == 3) {
        if(use_shape)
            return streaming_locate3<true>(query, positions, found_positions);
        else
            return streaming_locate3<false>(query, positions, found_positions);
    }
    else
        return 0;
}
    ...
    std::vector<std::pair<uint64_t, bool>> &positions, size_t &found_positions)
{
    ...
    for (auto&& window : query | rshash::views::kmerview({.window_size = window_size}))
    {
        ...
        if(minimiser == current_pos_minimiser)
            ...
            if (auto it = hashmap.find(std::min<uint64_t>(kmer, kmer_rc)); it != hashmap.end()) {
